...
function _ieval_args(f::ANY, args, argtypes::Vector{Any}, sv::VarInfo)
    c = cell(length(argtypes))
    c[1] = f
    for i = 2:length(argtypes)
        t = argtypes[i]
        if isType(t) && !has_typevars(t.parameters[1])
...
function pure_eval_call(f::ANY, fargs, argtypes::ANY, sv, e)
    ...
    args = _ieval_args(f, fargs, argtypes, sv)
...
function inlineable(f::ANY, ft::ANY, e::Expr, atypes::Vector{Any}, sv::VarInfo, enclosing_ast::Expr)
    ...
    if linfo.pure && isconstantargs(argexprs, atypes, sv)
        ...
        constargs = _ieval_args(f, argexprs, atypes, sv)
        try
            v = ccall(:jl_apply_generic, Any, (Ptr{Any}, UInt32), constargs, length(constargs))
